...
$servicos = $db->json(
	' 
	select * from cliente_x_servico cs
	inner join servico s on s.id_servico = cs.servico_id_servico
	where  cs.cliente_id_cliente = :id
	'
	,
	['id' => $id]
);
...
$fotos = $db->json(
	' 
	SELECT f.ds_foto as ds, f.img_foto as data from foto f
	where f.id_cliente = :id
	order by f.dt_foto desc
	'
	,
	['id' => $id]
);
...
$META_TITLE = strtoupper($clientes[0]["ds_cliente"]);

if (sizeof($clientes))
	$META_KEYWORDS = strlen($clientes[0]['palavras_chaves_cliente']) ? $clientes[0]['palavras_chaves_cliente'] : $META_KEYWORDS;
			...
			<div class="container">
				...
				<div class="section-title" data-aos="fade-left">
					<h2>
						...
						<?php echo $clientes[0]["ds_cliente"]; ?>
					</h2>
					<?php
					...
					if (sizeof($servicos)) {
						...
						<div>
							...
							<strong><i class="bi bi-wrench-adjustable-circle-fill"></i> </strong>
							<span class="upper">
								<?php
								$servicos = array_map(function ($e) {
									return $e['ds_servico'];
								}, $servicos);
								?>
								<a href="../../#servicos">
									<?php echo implode(" / ", $servicos) ?>
								</a>
							</span>
