...
$produtos = $db->json(
	'
	select s.*, f.img_foto as servico_img from produto s
	left JOIN foto f on f.id_produto = s.id_produto
	where s.id_produto = :id
	GROUP BY s.id_produto
	order by s.ds_produto, f.dt_foto desc
	'
	,
	['id' => $id]
);
...
$fotos = $db->json(
	...
	where id_produto = :id and home <> 1 and capa <> 1
...
if (sizeof($fotos))
	$META_PREVIEW = $META_URL . "/adm/uploads/imgs/" . $fotos[0]['data'];

$META_TITLE = strtoupper($produtos[0]["ds_produto"]);

if (sizeof($produtos))
	$META_KEYWORDS = strlen($produtos[0]['palavras_chaves_produto']) ? $produtos[0]['palavras_chaves_produto'] : $META_KEYWORDS;

//'https://itiny.live/evoluma/preview.webp'
//echo $imgPreview;
?>
<!DOCTYPE html>
<html lang="pt-BR">
			...
			<div class="container">
				...
				<div class="section-title" data-aos="fade-left">
					<h2>
						...
						<?php echo $produtos[0]["ds_produto"]; ?>
						<a class="back-arrow share" href="#" text="<?php echo $produtos[0]["ds_produto"]; ?>" title="Compartilhar">
							<i class="bi bi-share-fill" ></i>
						</a>
					</h2>
					<!-- <p>Empresa Orgulhosamente Brasileira</p> -->
				</div>
				...
				<div class="row botoes">
					...
					foreach ($produtos as $k => $v) {
						?>
						<div class="col  d-flex justify-content-center " data-aos="zoom-in">

							<div class="noticia-texto">
								<?php echo strJsonToHtml($v['texto_produto']); ?>
							</div>
				...
				<div class='shareDiv share' text='<?php echo $produtos[0]["ds_produto"]; ?>'>
